...
// Indirect call handler - matches Xenia's approach:
// - Null addresses are silently skipped (common for uninitialized CRT callbacks)
// - Out-of-range addresses are skipped
// - Missing functions are skipped
#define PPC_CALL_INDIRECT_FUNC(x) do { \
    uint32_t _addr = (uint32_t)(x); \
    if (_addr == 0) { \
        /* Null callback - silently skip like Xenia does */ \
    } else if (_addr >= PPC_CODE_BASE && _addr < PPC_CODE_BASE + PPC_CODE_SIZE) { \
        PPCFunc* _func = PPC_LOOKUP_FUNC(base, _addr); \
        if (_func != nullptr) { \
            _func(ctx, base); \
        } \
    } \
