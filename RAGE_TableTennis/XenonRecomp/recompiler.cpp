#include "pch.h"
#include "tabletennis_functions.h"
...
// GTA4-SPECIFIC FAST ANALYSIS - ALL FUNCTIONS HARDCODED, NO DISCOVERY
void Recompiler::Analyse()
{
    auto startTime = std::chrono::high_resolution_clock::now();
    fmt::println("GTA4 FAST MODE: Loading {} hardcoded functions...", TABLETENNIS_FUNCTION_COUNT);

    functions.reserve(TABLETENNIS_FUNCTION_COUNT + 500);

    // Load ALL 48,423 hardcoded GTA4 functions - NO ANALYSIS NEEDED
    for (size_t i = 0; i < TABLETENNIS_FUNCTION_COUNT; i++)
    {
        auto& [addr, size] = TABLETENNIS_FUNCTIONS[i];
        auto& fn = functions.emplace_back();
        fn.base = addr;
        fn.size = size;
        image.symbols.emplace(fmt::format("sub_{:X}", addr), addr, size, Symbol_Function);
    }
    fmt::println("  Loaded {} game functions", TABLETENNIS_FUNCTION_COUNT);


// Fix for duplicate helper functions - check if they already exist
for (size_t i = 14; i < 32; i++)
{
    if (config.restGpr14Address != 0) {
        uint32_t addr = config.restGpr14Address + (i - 14) * 4;
        // Check if already exists
        auto it = std::lower_bound(functions.begin(), functions.end(), addr,
            [](const Function& f, uint32_t a) { return f.base < a; });
        if (it == functions.end() || it->base != addr) {
            auto& f = functions.emplace_back();
            f.base = addr;
            f.size = (32 - i) * 4 + 12;
            image.symbols.emplace(fmt::format("__restgprlr_{}", i), f.base, f.size, Symbol_Function);
        }
    }
    if (config.saveGpr14Address != 0) {
        uint32_t addr = config.saveGpr14Address + (i - 14) * 4;
        auto it = std::lower_bound(functions.begin(), functions.end(), addr,
            [](const Function& f, uint32_t a) { return f.base < a; });
        if (it == functions.end() || it->base != addr) {
            auto& f = functions.emplace_back();
            f.base = addr;
            f.size = (32 - i) * 4 + 8;
            image.symbols.emplace(fmt::format("__savegprlr_{}", i), f.base, f.size, Symbol_Function);
        }
    }
    if (config.restFpr14Address != 0) {
        uint32_t addr = config.restFpr14Address + (i - 14) * 4;
        auto it = std::lower_bound(functions.begin(), functions.end(), addr,
            [](const Function& f, uint32_t a) { return f.base < a; });
        if (it == functions.end() || it->base != addr) {
            auto& f = functions.emplace_back();
            f.base = addr;
            f.size = (32 - i) * 4 + 4;
            image.symbols.emplace(fmt::format("__restfpr_{}", i), f.base, f.size, Symbol_Function);
        }
    }
    if (config.saveFpr14Address != 0) {
        uint32_t addr = config.saveFpr14Address + (i - 14) * 4;
        auto it = std::lower_bound(functions.begin(), functions.end(), addr,
            [](const Function& f, uint32_t a) { return f.base < a; });
        if (it == functions.end() || it->base != addr) {
            auto& f = functions.emplace_back();
            f.base = addr;
            f.size = (32 - i) * 4 + 4;
            image.symbols.emplace(fmt::format("__savefpr_{}", i), f.base, f.size, Symbol_Function);
        }
    }
}
    ...
    fmt::println("GTA4 FAST: {} total functions loaded in {:.3f} seconds", functions.size(), duration.count() / 1000.0f);
}
    ...
    CSRState& csrState)
{
    ...
    case PPC_INST_BCTR:
        if (switchTable != config.switchTables.end())
        {
            ...
            for (size_t i = 0; i < switchTable->second.labels.size(); i++)
            {
                ...
                {
                    // Check if this is a valid tail call to another function
                    auto it = std::lower_bound(functions.begin(), functions.end(), label,
                        [](const Function& f, uint32_t addr) { return f.base < addr; });
                    if (it != functions.end() && it->base == label)
                    {
                        // Valid tail call to another function
                        println("\t\tsub_{:X}(ctx, base);", label);
                        println("\t\treturn;");
                    }
                    else
                    {
    ...
    case PPC_INST_BDNZF:
        ...
        println("\t--{}.u64;", ctr());
        {
            ...
            if (target < fn.base || target >= fn.base + fn.size) {
                // Cross-function branch - emit function call
                auto it = std::lower_bound(functions.begin(), functions.end(), target,
                    [](const Function& f, uint32_t addr) { return f.base < addr; });
                if (it != functions.end() && it->base == target) {
                    println("\tif ({}.u32 != 0 \&\& !{}.eq) {{ sub_{:X}(ctx, base); return; }}", ctr(), cr(insn.operands[0] / 4), target);
                } else {
                    println("\tif ({}.u32 != 0 \&\& !{}.eq) goto loc_{:X}; // WARNING: cross-function", ctr(), cr(insn.operands[0] / 4), target);
                }
            } else {
                println("\tif ({}.u32 != 0 \&\& !{}.eq) goto loc_{:X};", ctr(), cr(insn.operands[0] / 4), target);
            }
        }
        break;
    ...
    case PPC_INST_VAVGUH:
        println("\tsimde_mm_store_si128((simde__m128i*){}.u8, simde_mm_avg_epu16(simde_mm_load_si128((simde__m128i*){}.u8), simde_mm_load_si128((simde__m128i*){}.u8)));", v(insn.operands[0]), v(insn.operands[1]), v(insn.operands[2]));
        break;
    ...
    case PPC_INST_DCBST:
        // no op - data cache instruction irrelevant on recompiled code
        break;
