...
void Recompiler::Analyse()
{
    ...
    fmt::println("GTA4 FAST MODE: Loading {} hardcoded functions...", MAXPAYNE_FUNCTION_COUNT);

    functions.reserve(MAXPAYNE_FUNCTION_COUNT + 500);

    // Load ALL 48,423 hardcoded GTA4 functions - NO ANALYSIS NEEDED
    for (size_t i = 0; i < MAXPAYNE_FUNCTION_COUNT; i++)
    {
        auto& [addr, size] = MAXPAYNE_FUNCTIONS[i];
        auto& fn = functions.emplace_back();
        fn.base = addr;
        fn.size = size;
        image.symbols.emplace(fmt::format("sub_{:X}", addr), addr, size, Symbol_Function);
    }
    fmt::println("  Loaded {} game functions", MAXPAYNE_FUNCTION_COUNT);
