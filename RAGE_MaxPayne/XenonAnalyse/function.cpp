...
size_t Function::SearchBlock(size_t address) const
{
    ...
    // First try exact match in hash map (O(1))
    auto it = blockMap.find(address - base);
    if (it != blockMap.end())
    {
        return it->second;
    }

    // Fall back to range search for addresses within blocks
...
size_t Function::AddBlock(size_t blockBase, size_t blockSize, size_t projectedSize)
{
    blocks.emplace_back(blockBase, blockSize, projectedSize);
    size_t index = blocks.size() - 1;
    blockMap[blockBase] = index;
    return index;
}
...
Function Function::Analyze(const void* code, size_t size, size_t base)
{
    ...
    // Use AddBlock for the first block to update hash map
    fn.AddBlock(0, 0);
    ...
    for (; data <= dataEnd ; ++data)
    {
        ...
        if (op == PPC_OP_BC) // conditional branches all originate from one opcode, thanks RISC
        {
            ...
            if (lBlock == -1)
            {
                lBlock = fn.AddBlock(lBase, 0, rBase - lBase);
            ...
            if (rBlock == -1)
            {
                rBlock = fn.AddBlock(branchDest - base, 0);
        ...
        else if (op == PPC_OP_B || instruction == 0 || (op == PPC_OP_CTR && (xop == 16 || xop == 528))) // b, blr, end padding
        {
            if (!isLink)
            {
                ...
                if (op == PPC_OP_B)
                {
                    ...
                    if (branchBlock == -1)
                    {
                        size_t newBlock = fn.AddBlock(branchBase, 0, sizeProjection);

                        blockStack.emplace_back(newBlock);
