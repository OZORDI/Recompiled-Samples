...
void Recompiler::Analyse()
{
    ...
    fmt::println("GTA4 FAST MODE: Loading {} hardcoded functions...", MCLA_FUNCTION_COUNT);

    functions.reserve(MCLA_FUNCTION_COUNT + 500);

    // Load ALL 48,423 hardcoded GTA4 functions - NO ANALYSIS NEEDED
    for (size_t i = 0; i < MCLA_FUNCTION_COUNT; i++)
    {
        auto& [addr, size] = MCLA_FUNCTIONS[i];
        auto& fn = functions.emplace_back();
        fn.base = addr;
        fn.size = size;
        image.symbols.emplace(fmt::format("sub_{:X}", addr), addr, size, Symbol_Function);
    }
    fmt::println("  Loaded {} game functions", MCLA_FUNCTION_COUNT);
